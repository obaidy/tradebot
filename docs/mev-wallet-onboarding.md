...
4. The runner now performs an RPC + balance preflight before every MEV job. If connectivity fails or the balance is under ~0.05 ETH, the run is halted and a warning is logged.
5. The portal monitors balance; if it drops below ~0.02 ETH you’ll see a warning. Top up before launching live runs.
6. Rotate or remove the key anytime via **Rotate key** or **Remove**. After rotating, fund the new address before restarting jobs.
7. For enterprise custody, coordinate with TradeBot support to set up managed key rotation if required.
