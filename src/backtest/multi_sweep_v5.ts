// src/backtest/multi_sweep_v5.ts
...
function simulate(candles: Candle[], params: any, starting = 200, stepSizeBase = 0.00001) {
  ...
  for (let i = 0; i < candles.length; i++) {
    ...
    // only open new positions if concurrency allows
    if (openPositions.length < maxConcurrent) {
      ...
      for (const lvl of buyLevels) {
        if (low <= lvl && cash >= perTradeUSD) {
          ...
          trades++;
          // immediate TP check if price action within same candle
    ...
    // check TP and stop for open positions
...
async function main() {
  ...
  const kArr = [0.5, 1];
  const tpArr = [0.10, 0.12, 0.15];
  const perTradeArr = [0.005, 0.01, 0.02];
  const meanRevertArr = [0.01];
  const minAtrArr = [0.002, 0.003];
  const rsiArr = [45, 50];
  const stopLossArr = [0.10, 0.15, 0.2];
  const maxConcurrentArr = [1, 2];

  const outPath = path.resolve(process.cwd(), "multi_sweep_v5.csv");
  const header =
    "pair,gridSteps,k,tp,perTrade,meanRevert,minAtr,rsi,stopLoss,maxConcurrent,trades,NAV,realized,maxDD,candles";
  const rows: string[] = [header];

  // --- robust stepSize detection (replace fragile single-line expression) ---
  let stepSize = 0.00001; // default fallback
  try {
    const ex: any = new ccxt.binance({ enableRateLimit: true });
    await ex.loadMarkets();
    // try direct lookup, fallback to searching by symbol
    const market =
    ...
    if (market) {
      if (market.precision && typeof market.precision.base === "number") {
        stepSize = Math.pow(10, -market.precision.base);
    ...
    console.warn("Could not detect stepSize from exchange; defaulting to", stepSize, err && err.message ? err.message : err);
  }
                ...
                for (const rsi of rsiArr)
                  for (const sl of stopLossArr)
                    for (const mc of maxConcurrentArr) {
                      ...
                      rows.push(
                        `${pair},${gs},${k},${tp},${pt},${mr},${minAtr},${rsi},${sl},${mc},${r.trades},${r.nav},${r.realized},${r.maxDD},${r.candles}`
                      );
                    }
  ...
  console.log("multi_sweep_v5 finished ->", outPath);
}
