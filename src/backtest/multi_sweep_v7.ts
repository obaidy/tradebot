// src/backtest/multi_sweep_v7.ts
...
function simulate(candles: Candle[], params: any, starting = 200, stepSizeBase = 0.00001) {
  ...
  for (let i = 0; i < candles.length; i++) {
    ...
    const mid = smaVals[i],
      atrVal = atrVals[i],
      price = closes[i];
    // loosened mean revert check - smaller meanRevertPct => more frequent triggers
...
async function main() {
  ...
  const meanRevertArr = [0.003, 0.005, 0.01]; // LOOSER: more triggers
  const minAtrArr = [0.001, 0.002, 0.003];
  const rsiArr = [45, 50];
  const stopLossArr = [0.08, 0.12, 0.15];
  const maxConcurrentArr = [1];

  const outPath = path.resolve(process.cwd(), "multi_sweep_v7.csv");
  const header =
  ...
  for (const pair of pairs) {
    ...
    for (const tf of timeframes) {
      const candles = await fetchOHLCV(pair, tf, 24 * 180);
  ...
  console.log("multi_sweep_v7 finished ->", outPath);
}
