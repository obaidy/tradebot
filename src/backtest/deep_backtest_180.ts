// src/backtest/deep_backtest_180.ts
...
async function fetchOHLCV(pair: string, timeframe = "1h", hours = 24 * 180): Promise<Candle[]> {
...
async function main() {
  const candidates = [
    { pair: "BTC/USDT", gridSteps: 8, k: 1, tp: 0.15, perTrade: 0.006, meanRevertPct: 0.01, minAtrPct: 0.003, rsiThresh: 50 },
    { pair: "BTC/USDT", gridSteps: 8, k: 0.5, tp: 0.15, perTrade: 0.01, meanRevertPct: 0.01, minAtrPct: 0.002, rsiThresh: 50 },
  ];

  const outPath = path.resolve(process.cwd(), "deep_backtest_180_results.csv");
  const lines = ["pair,timeframe,gridSteps,k,tp,perTrade,trades,nav,realized,maxDD,candles"];
  for (const c of candidates) {
    for (const tf of ["1h", "4h"]) {
      console.log("Fetching", c.pair, tf, "for 180d...");
      const candles = await fetchOHLCV(c.pair, tf, 24 * 180);
      console.log("Running sim", c.pair, tf, c);
      const res = simulate(candles, { ...c, smaPeriod: tf === "1h" ? 24 : 24 }, 200);
  ...
  console.log("Deep 180 backtest finished ->", outPath);
}
