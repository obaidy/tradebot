...
const MIGRATION_QUERIES: string[] = [
  ...
  `CREATE INDEX IF NOT EXISTS idx_bot_signals_client_bot ON bot_signals(client_id, bot_name, created_at DESC);`,
  `CREATE INDEX IF NOT EXISTS idx_bot_signals_type_ts ON bot_signals(client_id, signal_type, created_at DESC);`,
  `CREATE TABLE IF NOT EXISTS bot_positions (
      ...
      pnl_unrealized NUMERIC DEFAULT 0,
      meta JSONB,
      opened_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
    );`,
  `CREATE UNIQUE INDEX IF NOT EXISTS uniq_bot_positions_client_bot_symbol ON bot_positions(client_id, bot_name, venue, symbol);`,
  `CREATE TABLE IF NOT EXISTS whale_watchlist (
      id BIGSERIAL PRIMARY KEY,
      client_id TEXT NOT NULL REFERENCES clients(id) ON DELETE CASCADE,
      chain TEXT NOT NULL DEFAULT 'eth',
      wallet TEXT NOT NULL,
      min_liquidity_usd NUMERIC NOT NULL DEFAULT 30000,
      max_mcap_usd NUMERIC NOT NULL DEFAULT 5000000,
      blocked_tokens TEXT[] NOT NULL DEFAULT '{}'::TEXT[],
      notes TEXT,
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      UNIQUE (client_id, chain, wallet)
    );`,
  `CREATE TABLE IF NOT EXISTS sentiment_snapshots (
      id BIGSERIAL PRIMARY KEY,
      client_id TEXT NOT NULL REFERENCES clients(id) ON DELETE CASCADE,
      token TEXT NOT NULL,
      mentions_5m INTEGER NOT NULL,
      mentions_30m INTEGER NOT NULL,
      trending_score NUMERIC NOT NULL,
      liquidity_usd NUMERIC,
      dex_volume_5m NUMERIC,
      meta JSONB,
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
    );`,
  `CREATE INDEX IF NOT EXISTS idx_sentiment_snapshots_client_token ON sentiment_snapshots(client_id, token, created_at DESC);`,
  ...
  `CREATE INDEX IF NOT EXISTS idx_risk_events_client_type ON risk_events(client_id, type, created_at DESC);`,
